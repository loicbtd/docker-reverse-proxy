...
server {
    ...
    """ + ''.join([get_subfolder_location_config(subfolder_location_dict) for subfolder_location_dict in server_dict['subfolder_location_list']]) + """
}
"""
...
def start():
    if len(sys.argv) == 3:
        write_proxy_confs_files(get_dict_from_json_file(sys.argv[2]), sys.argv[1])
        stop()
